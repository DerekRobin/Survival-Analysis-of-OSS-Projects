---
title: "Bayesian survival analysis using host type as a predictor"
...
set.seed(42); 
require (tidyverse);
data <- read_csv('necessary_fields.csv')
...
exp_surv_model_surv_func <-
    ...
    mutate(`S(t|2)` = pmap(list(alpha, beta), function(a,b) {construct_survival_function(a,b,2)}),
           `S(t|1)` = pmap(list(alpha, beta), function(a,b) {construct_survival_function(a,b,1)}),
...
exp_surv_model_surv_func$`S(t|2)`[[1]](times[1:10])
```
```{r}
...
exp_surv_model_survival <-
    ...
    mutate(times_df = pmap(list(times_df, `S(t|2)`, `S(t|1)`, `S(t|0)`),
                           function(df, s2, s1, s0) {df %>% mutate(  s2 = s2(t),
                                                               s1 = s1(t),
                                                               s0 = s0(t))})) %>%
    select(-`S(t|2)`, -`S(t|1)`, -`S(t|0)`) %>%
    unnest(cols = c(times_df)) %>%
    gather(key = Strata, value = survival, s2, s1, s0) %>%
    mutate(Strata = factor(Strata, # Strata is for different host types
                              levels = c("s2","s1","s0"),
                              labels = c("host type=Git","host type=Pypi","host type=Debian")))
